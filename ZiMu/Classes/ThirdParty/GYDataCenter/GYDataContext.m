...
- (void)saveObject:(id<GYModelObjectProtocol>)object {
    ...
    [queue dispatchSync:^{
        ...
        NSMutableDictionary *cache = [self tableCacheFromDBCache:queue.cache class:modelClass];
        if (cache) {
            //setObjectForKey: key cannot be nil 的bug
            if ([(id)object valueForKey:[modelClass primaryKey]]) {
                [cache setObject:object forKey:[(id)object valueForKey:[modelClass primaryKey]]];
            }
        }
...
- (void)deleteTableWithModelClass:(Class<GYModelObjectProtocol>)modelClass {
    GYDataContextQueue *queue = [self queueForDBName:[modelClass dbName]];
    [queue dispatchAsync:^{
        [_dbRunner deleteTableWithModelClass:modelClass];
    }];
}
...
- (void)deleteTableWithTableName:(NSString *)tableName
    ...
    [queue dispatchAsync:^{
        [_dbRunner deleteTableWithTableName:tableName dbName:dbName];
    }];
}
...
- (void)deleteDatabaseWithModelClass:(Class<GYModelObjectProtocol>)modelClass {
    GYDataContextQueue *queue = [self queueForDBName:[modelClass dbName]];
    [queue dispatchAsync:^{
        [_dbRunner deleteDatabaseWithModelClass:modelClass];
    }];
}
...
- (void)deleteDatabaseWithdbName:(NSString *)dbName {
    GYDataContextQueue *queue = [self queueForDBName:dbName];
    [queue dispatchAsync:^{
        [_dbRunner deleteDatabaseWithdbName:dbName];
    }];
}
