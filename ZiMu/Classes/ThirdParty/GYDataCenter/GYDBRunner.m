...
#ifdef DEBUG
#define GYLog(s, ... ) NSLog( @"[%@ in line %d] ===>%@", [[NSString stringWithUTF8String:__FILE__] lastPathComponent], __LINE__, [NSString stringWithFormat:(s), ##__VA_ARGS__] )
#else
#define GYLog(s, ... )
#endif
...
- (void)deleteTableWithModelClass:(Class<GYModelObjectProtocol>)modelClass {
    GYDatabaseInfo *databaseInfo = [self databaseInfoForClass:modelClass];
    [databaseInfo.databaseQueue asyncInDatabase:^(FMDatabase *db) {
        NSString *sql = nil;
        sql = [NSString stringWithFormat:@"DROP TABLE IF EXISTS %@", [modelClass tableName]];
        [self recordWriteOperationForDatabaseInfo:databaseInfo];
...
- (void)deleteDatabaseWithModelClass:(Class<GYModelObjectProtocol>)modelClass {
    [self deleteDatabaseWithdbName:[modelClass dbName]];
}

- (void)deleteDatabaseWithdbName:(NSString *)dbName {
    NSString *path = [self pathForDbName:dbName];
    NSError *error;
    BOOL success;
...
- (NSString *)pathForDbName:(NSString *)dbName {
    ...
    NSLog(@"pathForDbName=%@", [documentsDirectory stringByAppendingPathComponent:[dbName stringByAppendingPathExtension:@"db"]]);
